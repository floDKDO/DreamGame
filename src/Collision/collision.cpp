...
std::optional<std::pair<glm::vec3, AABB>> detection(const gltf::Node* map_node, const gltf::Node* player_node)
{
	...
	for(const std::unique_ptr<gltf::Node>& child_node : map_node->get_children_nodes())
	{
		std::optional<std::pair<glm::vec3, AABB>> result = detection(child_node.get(), player_node);
		if(result.has_value())
		{
			return result;
		}
...
TranslationInfo response(const std::pair<glm::vec3, AABB>& collision_info, const gltf::Node* player_node)
{
	...
	{
		translation_info.axis_ = TranslationAxis::X;
		translation_info.value_ = (player_center.x < map_node_center.x) ? -overlap.x : overlap.x;
	}
	else if(overlap.y < overlap.z)
	{
		translation_info.axis_ = TranslationAxis::Y;
		translation_info.value_ = (player_center.y < map_node_center.y) ? -overlap.y : overlap.y;
	}
	else
	{
		translation_info.axis_ = TranslationAxis::Z;
		translation_info.value_ = (player_center.z < map_node_center.z) ? -overlap.z : overlap.z;
	}
