...
vec4 get_light_intensity(float square_size, float light_height)
{
	...
	{
		x_intensity = (1.0 - ((world_position_.x - min_square.x) / (max_square.x - min_square.x)));
	}
	else
	{
	...
	float z_intensity = 0.0; 
	if(world_position_.z > light_position_.z)
	{
		z_intensity = (1.0 - ((world_position_.z - min_square.z) / (max_square.z - min_square.z)));
	}
	else
	{
	...
	float final_intensity = (x_intensity + z_intensity) / 2.0;
	if(world_position_.y > light_position_.y + light_height)
	{
		final_intensity = 0.0;
	}
	return vec4(vec3(final_intensity), 1.0);
}
...
void main()
{
	...
	//diffuse
	vec4 light_intensity = get_light_intensity(12.0, 3.0);
